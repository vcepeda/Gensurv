...
const singleTotalSize = computed(() => getSubmissionSize(single.metadata, single.antibiotics, single.fastq));
const bulkTotalSize = computed(() => getSubmissionSize(bulk.metadata, bulk.antibiotics, bulk.fastq));
...
async function submitSingle() {
  const totalSize = getSubmissionSize(single.metadata, single.antibiotics, single.fastq);
  ...
  try {
    ...
    await apiClinet.post("/api/upload/single/?type=gensurv&dry_run=true", toPrecheckFormData(fd), {
...
async function submitBulk() {
  const totalSize = getSubmissionSize(bulk.metadata, bulk.antibiotics, bulk.fastq);
  if (totalSize > MAX_SUBMISSION_SIZE_BYTES) {
  ...
  try {
    ...
    await apiClinet.post("/api/upload/bulk/?type=gensurv&dry_run=true", toPrecheckFormData(fd), {
      headers: { "Content-Type": "multipart/form-data" },
