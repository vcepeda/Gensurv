<template>
  <li ref="rootEl" class="tree-node-item" :class="{ 'tree-node-highlighted': isTarget }">
    ...
    <template v-else>
      <a
        ...
        @click.prevent="emit('select-file', node)"
      >
        {{ node.name }}
      </a>
      <span v-else class="tree-file-name">{{ node.name }}</span>
      <i
        class="fas fa-download tree-download-icon"
        role="button"
        title="Download"
...
const emit = defineEmits(["select-file", "download-file"]);

// Kept in sync with PREVIEWABLE_EXTENSIONS in SubmissionSampleResultFilesView.vue
...
const isPreviewable = computed(
  () => !isDirectory.value && PREVIEWABLE_EXTENSIONS.has(extensionOf(props.node?.name || ""))
);
const expanded = ref(true);
const description = computed(() => STAGE_DESCRIPTIONS[props.node?.name?.toLowerCase()] || "");
const isTarget = computed(() => !!props.highlightPath && props.node?.path === props.highlightPath);
...
onMounted(() => {
  ...
  if (isTarget.value && rootEl.value) {
...
.tree-node-highlighted > .tree-dir-btn {
  background: #fff3cd;
