...
# ------------------------------------------------------------
# 2. COMPARE MODELS.PY USING compare_models.py
# Generates a structured markdown report of schema differences:
# tables added/removed, columns added/removed, foreign key changes,
# and a side by side comparison of all tables
# ------------------------------------------------------------
echo ""
echo "[2/5] Comparing models.py between branches..."

CURRENT_MODELS="$REPO_DIR/gensurvapp/models.py"
NEW_MODELS="$NEW_BRANCH_DIR/backend/gensurvapp/models.py"
REPORT_FILE="$LOGS_DIR/schema-comparison-$DATE.md"

# Run the comparison script using the mamba env python
$PYTHON_ENV "$(dirname "$0")/compare_models.py" \
  "$CURRENT_MODELS" \
  "$NEW_MODELS" \
  "$REPORT_FILE"
